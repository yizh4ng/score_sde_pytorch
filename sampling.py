...
class RTK(Predictor):
    ...
    def update_fn(self, x, t, h, x_prev=None):
        ...
        # one step ddim as initial state for langevin iteration
        sde = self.sde
        timestep = (t * (sde.N - 1) / sde.T).int().to(x.device)
        timestep_next = ((t - h) * (sde.N - 1) / sde.T).int().to(x.device)  # same exact thing as  timestep - 1
        # print(timestep, timestep_next)
...
@register_predictor(name='rtk1')
class RTK1(Predictor):
    """The ancestral sampling predictor. Currently only supports VE/VP SDEs."""
    ...
    def update_fn(self, x, t, h, x_prev=None):
        ...
        for _ in range(1):
            ...
            # score = self.score_fn(x, t)
            term_1 = -(-2 * current_x * torch.exp(-_h)) / weight
            term_2 = -(2 * x * torch.exp(-2 * _h)) / weight
